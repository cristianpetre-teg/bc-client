...
import type { TimelineResultBean } from './TimelineResultBean.js';
import {
    TimelineResultBeanFromJSON,
    TimelineResultBeanFromJSONTyped,
    TimelineResultBeanToJSON,
} from './TimelineResultBean.js';

/**
 * 
 * @export
 * @interface BulkResultBean
 */
export interface BulkResultBean {
    /**
     * The identifier (optionally) as passed in the input, can be used as reference.
     * @type {string}
     * @memberof BulkResultBean
     */
    identifier?: string;
    /**
     * The profile ID of the profile that was created, updated or deleted.
     * @type {string}
     * @memberof BulkResultBean
     */
    profileId?: string;
    /**
     * The possible states for profile related changes.
     * @type {string}
     * @memberof BulkResultBean
     */
    state?: BulkResultBeanStateEnum;
    /**
     * The feedback on the given timeline events.
     * @type {Array<TimelineResultBean>}
     * @memberof BulkResultBean
     */
    timeline?: Array<TimelineResultBean>;
    /**
     * The errors found for the given profile changes (if any).
     * @type {{ [key: string]: any; }}
     * @memberof BulkResultBean
     */
 ...
 * @export
 */
export const BulkResultBeanStateEnum = {
    Created: 'CREATED',
    Skipped: 'SKIPPED',
    Modified: 'MODIFIED',
    Unchanged: 'UNCHANGED',
    Notfound: 'NOTFOUND',
    RestrictionMismatch: 'RESTRICTION_MISMATCH',
    ConsentMismatch: 'CONSENT_MISMATCH',
    Deleted: 'DELETED'
} as const;
export type BulkResultBeanStateEnum = typeof BulkResultBeanStateEnum[keyof typeof BulkResultBeanStateEnum];


/**
 * Check if a given object implements the BulkResultBean interface.
 */
export function instanceOfBulkResultBean(value: object): boolean {
    return true;
}

export function BulkResultBeanFromJSON(json: any): BulkResultBean {
    return BulkResultBeanFromJSONTyped(json, false);
}

export function BulkResultBeanFromJSONTyped(json: any, ignoreDiscriminator: boolean): BulkResultBean {
    if (json == null) {
        return json;
    }
    return {
        ...
        'timeline': json['timeline'] == null ? undefined : ((json['timeline'] as Array<any>).map(TimelineResultBeanFromJSON)),
        'validationErrors': json['validationErrors'] == null ? undefined : json['validationErrors'],
    };
}

export function BulkResultBeanToJSON(value?: BulkResultBean | null): any {
    if (value == null) {
        return value;
    }
    return {
        ...
        'state': value['state'],
        'timeline': value['timeline'] == null ? undefined : ((value['timeline'] as Array<any>).map(TimelineResultBeanToJSON)),
