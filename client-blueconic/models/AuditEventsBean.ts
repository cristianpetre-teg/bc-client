...
import type { AuditEntryBean } from './AuditEntryBean.js';
import {
    AuditEntryBeanFromJSON,
    AuditEntryBeanFromJSONTyped,
    AuditEntryBeanToJSON,
} from './AuditEntryBean.js';

/**
 * 
 * @export
 * @interface AuditEventsBean
 */
export interface AuditEventsBean {
    /**
     * 
     * @type {Array<AuditEntryBean>}
     * @memberof AuditEventsBean
     */
    auditEvents?: Array<AuditEntryBean>;
}

/**
 * Check if a given object implements the AuditEventsBean interface.
 */
export function instanceOfAuditEventsBean(value: object): boolean {
    return true;
}

export function AuditEventsBeanFromJSON(json: any): AuditEventsBean {
    return AuditEventsBeanFromJSONTyped(json, false);
}

export function AuditEventsBeanFromJSONTyped(json: any, ignoreDiscriminator: boolean): AuditEventsBean {
    if (json == null) {
        return json;
    }
    return {
        
        'auditEvents': json['auditEvents'] == null ? undefined : ((json['auditEvents'] as Array<any>).map(AuditEntryBeanFromJSON)),
    };
}

export function AuditEventsBeanToJSON(value?: AuditEventsBean | null): any {
    if (value == null) {
        return value;
    }
    return {
        
        'auditEvents': value['auditEvents'] == null ? undefined : ((value['auditEvents'] as Array<any>).map(AuditEntryBeanToJSON)),
    };
}
