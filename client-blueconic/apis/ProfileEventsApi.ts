 ...
 * 
 */
export class ProfileEventsApi extends runtime.BaseAPI {

    /**
     * Retrieves the consent management events for a profile; consent changed or permission level changed.
     * Get profile events
     */
    async get1Raw(requestParameters: Get1Request, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<ProfileEvents>> {
        if (requestParameters['profileId'] == null) {
            throw new runtime.RequiredError(
                'profileId',
                'Required parameter "profileId" was null or undefined when calling get1().'
            );
        }
        ...
        const response = await this.request({
            path: `/profileEvents/{profileId}`.replace(`{${"profileId"}}`, encodeURIComponent(String(requestParameters['profileId']))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => ProfileEventsFromJSON(jsonValue));
    }
     ...
     * Get profile events
     */
    async get1(requestParameters: Get1Request, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<ProfileEvents> {
